...
exports.POST = (req, res) => {
  ...
  const sql = "SELECT employerid FROM employers WHERE companyname = ? LIMIT 1;";
  db.query(sql, [companyname], (err, result) => {
    ...
    const sql = "SELECT employerid FROM employers WHERE phone = ? LIMIT 1;";
    db.query(sql, [phone], (err, result) => {
      ...
      const sql = "SELECT employerid FROM employers WHERE website = ? LIMIT 1;";
        ...
        db.query(
          ...
          (err, result) => {
            if (err) {
              console.log(err);
              return res.status(500).send({
                msg: "unable to insert new employer",
                msgType: "error",
              });
            }

            return res.status(200).send({
              msg: "employer added successfully",
              msgType: "success",
