...
exports.POST = (req, res) => {
  ...
  const sql =
    "SELECT providerid FROM providers WHERE companyname = ? AND providerid <> ? LIMIT 1;";
  db.query(sql, [companyname, providerid], (err, result) => {
    if (err) {
      console.log(err);
    ...
    const sql =
      "SELECT providerid FROM providers WHERE phone = ? AND providerid <> ? LIMIT 1;";
    db.query(sql, [phone, providerid], (err, result) => {
      if (err) {
        console.log(err);
        return res.status(500).send({
      ...
      const sql =
        "SELECT providerid FROM providers WHERE website = ? AND providerid <> ? LIMIT 1;";
      db.query(sql, [website, providerid], (err, result) => {
        if (err) {
          console.log(err);
          return res.status(500).send({
