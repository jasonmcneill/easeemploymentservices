...
exports.POST = (req, res) => {
  ...
  const sql =
    "SELECT employerid FROM employers WHERE companyname = ? AND employerid <> ? LIMIT 1;";
  db.query(sql, [companyname, employerid], (err, result) => {
    if (err) {
      console.log(err);
    ...
    const sql =
      "SELECT employerid FROM employers WHERE phone = ? AND employerid <> ? LIMIT 1;";
    db.query(sql, [phone, employerid], (err, result) => {
      if (err) {
        console.log(err);
        return res.status(500).send({
      ...
      const sql =
        "SELECT employerid FROM employers WHERE website = ? AND employerid <> ? LIMIT 1;";
      db.query(sql, [website, employerid], (err, result) => {
        if (err) {
          console.log(err);
          return res.status(500).send({
